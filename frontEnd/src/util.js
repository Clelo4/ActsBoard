...
export default {
  getCode: () => {
    ...
    console.log(this.a);
    const code = this.a.getUrlParam("code"); //这个getUrlParam需要自己实现
    ...
    if (if_open_id < 0) {
      if (code == null || code === "") {
        window.location.href = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + AppId + "&redirect_uri=http://web.iamxuyuan.com&response_type=code&scope=snsapi_base&connect_redirect=1#wechat_redirect"; //这是要调用的微信官方接口,拿到回调的url，相当于重新刷新了页面
      } else {
